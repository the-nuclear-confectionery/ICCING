...
Event IO::InitializeEvent()
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    event_in.momentum.push_back(mom);
  }
...
void IO::OutputEccentricities(double total_entropy, vector<vector<double>> eccentricities, string density_type, string file_name)
{
  ...
  if (density_type == "Energy")
  {
    ...
    {
      output << eccentricities[i][0] << " " << eccentricities[i][1] << " " << eccentricities[i][2] << " ";
    }
  ...
  else if (density_type == "Charge")
  {
    ...
    for (int i = 0; i < eccentricities.size(); i++)
    {
      output_neg << eccentricities[i][0] << " " << eccentricities[i][1] << " " << eccentricities[i][2] << " ";
      output_pos << eccentricities[i][3] << " " << eccentricities[i][4] << " " << eccentricities[i][5] << " ";
    }
...
//  cout << "testing eccentricities " << event.initial_eccentricities[0][0] << " " << event.initial_eccentricities[0][1] << " " << event.initial_eccentricities[0][2] << endl;
  ...
  if (test_ != "ConvertEvent")
  {
      ...
      OutputEccentricities(event.total_initial_entropy, event.estimator_integrals[0], "Energy", output_dir + "baryon_estimator_integrals");
      OutputEccentricities(event.total_initial_entropy, event.estimator_integrals[1], "Energy", output_dir + "strange_estimator_integrals");
      OutputEccentricities(event.total_initial_entropy, event.estimator_integrals[2], "Energy", output_dir + "charge_estimator_integrals");
  }
