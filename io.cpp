...
//  cout << "testing eccentricities " << event.initial_eccentricities[0][0] << " " << event.initial_eccentricities[0][1] << " " << event.initial_eccentricities[0][2] << endl;
  ...
  if (test_ != "ConvertEvent")
  {
      ...
      OutputEstimatorIntegrals(event.total_initial_entropy, event.estimator_integrals[0], "Energy", output_dir + "baryon_estimator_integrals");
      OutputEstimatorIntegrals(event.total_initial_entropy, event.estimator_integrals[1], "Energy", output_dir + "strange_estimator_integrals");
      OutputEstimatorIntegrals(event.total_initial_entropy, event.estimator_integrals[2], "Energy", output_dir + "charge_estimator_integrals");
  }
